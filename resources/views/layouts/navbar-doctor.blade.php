<header class="w-full bg-white/95 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-50 transition-all duration-300">
    <div class="px-4 sm:px-6 md:px-8 lg:px-10 py-0 h-20 flex items-center justify-between">
        ...
        <a href="{{ route('dokter.dashboard') }}" class="flex items-center gap-3 decoration-none text-none">
        ...
        <nav class="hidden md:flex items-center gap-6 text-sm font-semibold">
            <a href="{{ route('dokter.dashboard') }}" class="relative group transition-all duration-200 ease-in-out {{ request()->routeIs('dokter.dashboard') ? 'text-blue-600 font-semibold' : 'text-slate-600 hover:text-blue-600' }}">
                Beranda
                <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-blue-600 opacity-0 transition-all duration-200 ease-in-out group-hover:opacity-100 {{ request()->routeIs('dokter.dashboard') ? 'opacity-100' : '' }}"></span>
            </a>
            <a href="{{ route('dokter.reservasi.history') }}" class="relative group transition-all duration-200 ease-in-out {{ request()->routeIs('dokter.reservasi.history') ? 'text-blue-600 font-semibold' : 'text-slate-600 hover:text-blue-600' }}">
                Riwayat Reservasi
                <span class="absolute left-0 -bottom-1 h-0.5 w-full bg-blue-600 opacity-0 transition-all duration-200 ease-in-out group-hover:opacity-100 {{ request()->routeIs('dokter.reservasi.history') ? 'opacity-100' : '' }}"></span>
            </a>
                ...
                <div id="bellDropdown" class="absolute right-0 mt-2 w-80 bg-white border border-slate-100 rounded-2xl shadow-xl p-3 hidden opacity-0 scale-95 transition-all duration-200 ease-in-out origin-top-right z-50" style="display: none;" role="menu" aria-label="Notifikasi">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500 mt-1">Ringkasan aktivitas terkini Anda.</p>
                        </div>
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('dokter.reservasi.history') }}" class="block rounded-2xl border border-slate-100 bg-slate-50/80 p-3 hover:bg-slate-100 transition-all duration-200 ease-in-out">
                            <div class="flex items-center justify-between gap-2 mb-1">
                                <p class="text-[12px] font-semibold text-slate-800">Jadwal baru diterima</p>
                                <span class="inline-flex h-2.5 w-2.5 rounded-full bg-rose-500"></span>
                            </div>
                            <p class="text-[11px] text-slate-500">Reservasi pasien baru telah menunggu konfirmasi.</p>
                        </a>
                        <div class="rounded-2xl border border-slate-100 p-3 hover:bg-slate-50 transition-all duration-200 ease-in-out">
                            <div class="flex items-center justify-between gap-2 mb-1">
                                <p class="text-[12px] font-semibold text-slate-800">Janji besok pagi</p>
                                <span class="text-[10px] text-slate-400">1 hari lalu</span>
                            </div>
                            <p class="text-[11px] text-slate-500">Pastikan status jadwal besok telah diperbarui.</p>
                        </div>
                        <div class="rounded-2xl border border-slate-100 p-3 hover:bg-slate-50 transition-all duration-200 ease-in-out">
                            <div class="flex items-center justify-between gap-2 mb-1">
                                <p class="text-[12px] font-semibold text-slate-800">Rujukan BPJS aktif</p>
                                <span class="text-[10px] text-slate-400">3 hari lalu</span>
                            </div>
                            <p class="text-[11px] text-slate-500">Integrasi rujukan BPJS Kesehatan berhasil tersinkron.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="relative">
                <button type="button" id="profileBtn" aria-haspopup="true" aria-expanded="false"
                    ...
                    <div class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 font-bold text-xs flex items-center justify-center border border-blue-100 uppercase">
                        {{ substr(Auth::user()->name ?? 'DR', 0, 2) }}
                    </div>

                    <div class="text-left hidden sm:block">
                        <span class="text-xs font-extrabold text-slate-800 block leading-tight">{{ Auth::user()->name ?? 'Dokter' }}</span>
                        <span class="text-[9px] text-slate-400 font-bold uppercase block mt-0.5 tracking-wider">
                            ID: {{ Auth::user()->id ?? 'DOC-0014' }}
                        </span>
                    </div>
                ...
                <div id="profileDropdown" class="hidden absolute right-0 mt-2 w-64 bg-white border border-slate-100 rounded-2xl shadow-xl p-3 opacity-0 scale-95 transition-all duration-200 ease-in-out origin-top-right z-50" style="display: none;" aria-label="Menu Profil">
                    ...
                    <a href="{{ route('dokter.reservasi.history') }}" class="flex items-center gap-3 rounded-2xl px-3 py-2 text-slate-700 hover:bg-slate-50 transition-all duration-200 ease-in-out">
    ...
    <div class="md:hidden border-t border-slate-100 bg-white px-4 py-3 hidden" id="mobileMenuPanel" style="display: none;">
        <div class="flex flex-col gap-2">
            <a href="{{ route('dokter.dashboard') }}" class="bg-blue-50 text-blue-600 px-3 py-2.5 rounded-lg text-xs font-bold">Beranda</a>
            <a href="{{ route('dokter.reservasi.history') }}" class="text-slate-600 hover:bg-slate-50 px-3 py-2.5 rounded-lg text-xs transition-all duration-200 ease-in-out">Riwayat Reservasi</a>
            <a href="{{ route('dokter.dashboard') }}" class="text-slate-600 hover:bg-slate-50 px-3 py-2.5 rounded-lg text-xs transition-all duration-200 ease-in-out">Statistik Harian</a>
